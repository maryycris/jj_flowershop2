...
<div class="container py-3">
	<div class="d-flex justify-content-between align-items-center mb-3">
		...
		<a href="{{ route('clerk.loyalty.index') }}" class="btn btn-outline-secondary">← Back to Cards</a>
	</div>
					...
					@php
						...
						foreach($card->adjustments as $adjustment) {
							$activities->push([
								...
								'description' => 'Manual adjustment by ' . optional($adjustment->adjustedBy)->name . ($adjustment->reason ? (' — ' . $adjustment->reason) : ''),
						...
						<div class="timeline">
							@foreach($activities as $activity)
								<div class="d-flex align-items-start mb-3">
									<div class="me-3">
										<div class="timeline-marker d-flex align-items-center justify-content-center rounded-circle {{ $activity['type'] === 'stamp' ? 'bg-success text-white' : ($activity['type'] === 'redemption' ? 'bg-warning' : 'bg-info') }}">
									...
									<div class="flex-grow-1">
										<div class="d-flex justify-content-between align-items-start">
											...
											<div class="text-end">
												...
												@else
													<span class="badge {{ $activity['delta'] > 0 ? 'bg-success' : 'bg-warning' }}">{{ $activity['delta'] > 0 ? '+' : '' }}{{ $activity['delta'] }}</span>
												@endif
											</div>
										</div>
									</div>
