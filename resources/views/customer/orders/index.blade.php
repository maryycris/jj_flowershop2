                            ...
                            @foreach ($orders as $order)
                                @foreach ($order->products as $product)
                                    <div class="row order-list-row order-row" 
                                        data-status="{{ \App\Services\OrderStatusService::getCustomerDisplayStatus($order->status) }}"
                                        ...
                                        <div class="col-md-1 d-flex align-items-center justify-content-center">
                                            <img src="{{ $product->image_url ?? asset('images/placeholder.png') }}" class="order-product-img" alt="Product">
                                        </div>
                                        <div class="col-md-3 d-flex flex-column justify-content-center">
                                            <div class="fw-bold">{{ $product->name }}</div>
                                            <div class="text-muted small">Order #{{ $order->id }}</div>
                                        </div>
                                        <div class="col-md-2 d-flex align-items-center">₱{{ number_format($product->price, 2) }}</div>
                                        <div class="col-md-2 d-flex align-items-center">x{{ $product->pivot->quantity }}</div>
                                        <div class="col-md-2 d-flex align-items-center">{{ $order->created_at->format('M d, Y') }}</div>
                                        <div class="col-md-2 d-flex align-items-center justify-content-end">
                                            @if($order->status === 'pending')
                                                <span class="btn btn-sm btn-warning order-status-btn" style="font-weight:bold;">Pending Approval</span>
                                            @else
                                                <span class="btn btn-sm btn-outline-success order-status-btn">{{ \App\Services\OrderStatusService::getStatusLabel(\App\Services\OrderStatusService::getCustomerDisplayStatus($order->status)) }}</span>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div id="reviewList" style="display:none;">
                        <div class="bg-white rounded shadow-sm p-3">
                            ...
                            <div id="toBeReviewSection">
                                @foreach($toBeReview as $item)
                                <div class="row order-list-row align-items-center">
                                    <div class="col-md-6 d-flex align-items-center">
                                        <img src="{{ $item['image'] }}" class="order-product-img me-3" alt="Product">
                                        <div>
                                            <div class="fw-bold">{{ $item['name'] }}</div>
                                            <div class="text-muted small">Order {{ $item['order'] }}</div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <form>
                                            <div class="star-rating mb-2">
                                                <input type="radio" id="star5-{{ $loop->index }}" name="rating-{{ $loop->index }}" value="5"><label for="star5-{{ $loop->index }}">★</label>
                                                <input type="radio" id="star4-{{ $loop->index }}" name="rating-{{ $loop->index }}" value="4"><label for="star4-{{ $loop->index }}">★</label>
                                                <input type="radio" id="star3-{{ $loop->index }}" name="rating-{{ $loop->index }}" value="3"><label for="star3-{{ $loop->index }}">★</label>
                                                <input type="radio" id="star2-{{ $loop->index }}" name="rating-{{ $loop->index }}" value="2"><label for="star2-{{ $loop->index }}">★</label>
                                                <input type="radio" id="star1-{{ $loop->index }}" name="rating-{{ $loop->index }}" value="1"><label for="star1-{{ $loop->index }}">★</label>
                                            </div>
                            ...
                            <div id="reviewedSection" style="display:none;">
                                ...
                                <div class="row order-list-row align-items-center">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="review-comment-box">{{ $item['comment'] }}</div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
