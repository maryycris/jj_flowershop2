...
<div class="py-4" style="background: #f4faf4; min-height: 100vh;">
    ...
    <form action="<?php echo e(route('customer.checkout.process')); ?>" method="POST" id="paymentForm">
        <?php echo csrf_field(); ?>
        <input type="hidden" name="recipient_type" value="<?php echo e(session('checkout_data.recipient_type', 'someone')); ?>">
        <input type="hidden" name="delivery_address" value="<?php echo e(session('checkout_data.delivery_address', '')); ?>">
        <input type="hidden" name="recipient_name" value="<?php echo e(session('checkout_data.recipient_name', '')); ?>">
        <input type="hidden" name="recipient_phone" value="<?php echo e(session('checkout_data.recipient_phone', '')); ?>">
        <input type="hidden" name="delivery_date" value="<?php echo e(session('checkout_data.delivery_date', '')); ?>">
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-12 col-lg-8 col-xl-6" style="max-width: 1200px;">
                <div class="bg-white rounded-3 p-3 mb-4 scrollable-content" style="box-shadow: none; max-height: 85vh; overflow-y: auto;">
                    <div class="mb-3">
                        <a href="<?php echo e(route('customer.checkout.index')); ?>" class="btn btn-outline-success">
                    ...
                    ?>
                    <div class="mb-3 small text-muted">Loyalty Stamps: <strong><?php echo e($stamps); ?>/5</strong></div>
                    <?php if($stamps >= 5): ?>
                    ...
                    <?php if(session('checkout_data.delivery_date') && session('checkout_data.delivery_time')): ?>
                    <div class="mb-4">
                        <div class="p-3" style="background: linear-gradient(135deg, #e8f5e8, #f0f8f0); border-radius: 8px; border-left: 4px solid #8ACB88;">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <span class="fw-semibold"><?php echo e(\Carbon\Carbon::parse(session('checkout_data.delivery_date'))->format('M d, Y')); ?></span>
                                </div>
                                <div class="col-md-6">
                                    <small class="text-muted d-block">Delivery Time</small>
                                    <span class="fw-semibold"><?php echo e(session('checkout_data.delivery_time')); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12 col-lg-4 col-xl-4">
                <div class="bg-white rounded-3 p-3 mb-4" style="box-shadow: none;">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <img src="<?php echo e(asset('storage/' . $item->product->image)); ?>" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;">
                        <div class="flex-grow-1 ms-3">
                            <div style="font-weight: 500; color: #222;"><?php echo e($item->product->name); ?></div>
                            <div style="font-size: 0.9rem; color: #666;">Quantity: <?php echo e($item->quantity); ?></div>
                        </div>
                        <div class="text-end">
                            <div style="font-weight: 600; color: #222;">₱<?php echo e(number_format($item->quantity * $item->product->price, 2)); ?></div>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <div class="mb-3">
                        <div class="card" style="border: 2px solid #e8f5e8; background: linear-gradient(135deg, #f8f9fa, #e8f5e8);">
                            <div class="card-body p-3">
                                ...
                                <?php if($loyaltyCard && $loyaltyCard->stamps_count >= 4): ?>
                                    <div class="text-center">
                                        ...
                                        <?php if($loyaltyDiscount > 0): ?>
                                            ...
                                            <div class="alert alert-success py-2 px-3 mb-2">
                                                ...
                                                <strong><?php echo e($discountedItem->product->name); ?></strong> - 50% OFF!
                                            </div>
                                        <?php else: ?>
                                            <p class="text-success fw-bold mb-2">🎉 You have <?php echo e($loyaltyCard->stamps_count); ?>/5 stamps! 50% discount will be applied automatically!</p>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <div class="d-flex justify-content-between mb-2">
                        <span style="color: #666;">Subtotal (<?php echo e(count($cartItems)); ?> Item<?php echo e(count($cartItems) == 1 ? '' : 's'); ?>)</span>
                        <span style="color: #222;">₱<?php echo e(number_format($subtotal, 2)); ?></span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                        <span style="color: #666;">Shipping Fee</span>
                        <span style="color: #222;">₱<?php echo e(number_format($shippingFee, 2)); ?></span>
                    </div>
                    <?php if($loyaltyDiscount > 0): ?>
                    <div class="d-flex justify-content-between mb-2">
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <span style="color: #7bb47b; font-weight: 600; font-size: 1.2rem;">₱<?php echo e(number_format($finalTotal, 2)); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
<?php echo $__env->make('layouts.customer_app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\JJ_Flowershop_Capstone\resources\views/customer/checkout/payment_method.blade.php ENDPATH**/ ?>