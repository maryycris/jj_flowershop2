                ...
                <div class="d-flex align-items-center justify-content-between border-bottom pb-1">
                    <div class="d-flex align-items-center justify-content-center gap-5" style="padding: 0 4.0vw;">
                        <div class="d-flex align-items-center justify-content-center gap-5 pb-1" style="margin-left: 230px;">
                            <a href="<?php echo e(route('customer.dashboard')); ?>" class="nav-link text-white d-flex align-items-center gap-1 <?php if(request()->routeIs('customer.dashboard')): ?> active <?php endif; ?>"><i class="bi bi-house-door"></i> Home</a>
                            <a href="<?php echo e(route('customer.products.customize')); ?>" class="nav-link text-white d-flex align-items-center gap-1 <?php if(request()->routeIs('products.customize')): ?> active <?php endif; ?>"><i class="bi bi-brush"></i> Customize</a>
                            <a href="<?php echo e(route('customer.notifications.index')); ?>" class="nav-link text-white d-flex align-items-center gap-1 position-relative <?php if(request()->routeIs('customer.notifications.index')): ?> active <?php endif; ?>">
                                <i class="bi bi-bell"></i> Notifications
                                <?php if(isset($unreadCount) && $unreadCount > 0): ?>
                                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size: 0.8rem;"><?php echo e($unreadCount); ?></span>
                                <?php endif; ?>
                            </a>
                            <div class="dropdown">
                                <button class="nav-link text-white d-flex align-items-center gap-1 btn btn-link p-0" type="button" id="customerUserDropdown" data-bs-toggle="dropdown" aria-expanded="false" style="font-size: 1rem; background: none; border: none;">
                                    <i class="bi bi-person-circle" style="font-size: 1rem;"></i> <?php echo e(Auth::user()->name ?? "customer's name"); ?>
    ...
    <script>
        ...
        function sendMessage() {
            ...
            if (msg) {
                ...
                // Send to backend
                fetch("<?php echo e(route('customer.chat.send')); ?>", {
                    method: 'POST',
                    headers: {
        ...
        function loadMessages() {
            fetch("<?php echo e(route('customer.chat.messages')); ?>")
            .then(response => response.json())
            .then(data => {
                if (data.success && data.messages.length > 0) {
                    ...
                    data.messages.forEach(msg => {
                        ...
                        div.className = msg.sender_id == <?php echo e(Auth::id()); ?> ? 'chat-message' : 'chat-message support';
